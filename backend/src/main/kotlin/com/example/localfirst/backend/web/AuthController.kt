...
import com.example.localfirst.backend.auth.AuthSession
...
class AuthController(private val auth: AuthService) {
    @PostMapping("/codes")
    fun requestCode(@RequestBody request: CodeRequest) = auth
        .requestCode(request.contact, request.purpose)
        .let { CodeResponse(it.developmentCode, it.expiresAtMillis) }

    @PostMapping("/register")
    fun register(@RequestBody request: RegisterRequest) = auth
        .register(request.contact, request.password, request.code)
        .response()

    @PostMapping("/login")
    fun login(@RequestBody request: LoginRequest) = auth
        .login(request.contact, request.password)
        .response()

    @PostMapping("/reset-password")
    fun reset(@RequestBody request: ResetPasswordRequest) = auth
        .resetPassword(request.contact, request.newPassword, request.code)

    @GetMapping("/me")
    fun me(@RequestHeader("Authorization", required = false) authorization: String?) = auth
        .authenticate(authorization)
        .let { UserResponse(it.id, it.contact, it.createdAtMillis) }
