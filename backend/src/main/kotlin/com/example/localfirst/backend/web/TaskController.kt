...
class TaskController(
    ...
    @GetMapping("/{taskId}")
    fun task(@RequestHeader("Authorization", required=false) authorization:String?, @PathVariable taskId: String): ServerTask {
        if(authorization.isNullOrBlank()&&allowLegacySync)return tasks.find(taskId)?:throw TaskNotFoundException()
        val user = auth.authenticate(authorization)
        return tasks.find(scopedTaskId(user.id, taskId))?.copy(id = taskId) ?: throw TaskNotFoundException()
    }

    @GetMapping
    fun tasks(@RequestHeader("Authorization", required=false) authorization:String?): List<ServerTask> {
        if(authorization.isNullOrBlank()&&allowLegacySync)return tasks.listByPrefix("")
        val user = auth.authenticate(authorization)
        return tasks.listByPrefix(taskPrefix(user.id)).map { it.copy(id = unscopedTaskId(user.id, it.id)) }
    }
}
